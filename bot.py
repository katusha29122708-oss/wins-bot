...
def classify_message(text):
    result = ai(
        "Определи тип сообщения пользователя. Отвечай ТОЛЬКО одним словом из трёх: win, struggle, question.
"
        "win — человек описывает конкретное действие которое УЖЕ совершил: выпил кофе, погулял, поел, лёг, посмотрел сериал.
"
        "struggle — человек говорит что ничего не делал, устал, плохо себя чувствует, жалуется на день.
"
        "question — человек задаёт вопрос, спрашивает что-то, просит показать итог или статистику.
"
        "ВАЖНО: если это вопрос — отвечай question, не win.
"
...
def reframe_win(text):
    result = ai(
        ...
        "Если действий несколько — перечисли через новую строку, каждое начинается с символа •. "
        "Без пафоса, без слов молодец и браво. Только на русском, никаких иероглифов.",
        text
    )
...
def followup_question(count):
    result = ai(
        ...
        "Например: что ещё сегодня было? или а ещё что-нибудь? "
        "Только один вопрос, одно короткое предложение. На русском.",
        f"записана победа номер {count}",
        max_tokens=40
    )
...
def respond_to_struggle(text):
    result = ai(
        ...
        "Важно: это нормально — иногда отдыхать и ничего не делать тоже нужно. "
        "Не жалей человека и не драматизируй. Ответь легко и тепло — признай что отдых это ок. "
        "Затем задай ОДИН простой вопрос чтобы вспомнить хоть что-то из дня — "
        "что ел, что смотрел, с кем говорил, куда выходил. "
...
def handle_message(user_id, text):
    ...
    if text.lower() in ["/старт", "/start", "начать"]:
        send(user_id,
             ...
             "Если напишешь что ничего не делала — не буду записывать это как победу, просто поговорим.\n\n"
             "Команды:\n"
             "/итог — всё за сегодня\n"
