...
def classify_message(text):
    system = (
        ...
        "Отвечай ТОЛЬКО одним словом: win, struggle или question. "
        "win — человек описывает конкретное действие которое уже совершил: выпил кофе, погулял, поел, посмотрел сериал, лежал. "
        "struggle — человек говорит что ничего не делал, устал, плохо себя чувствует, день был пустым. "
        "question — человек задаёт вопрос или спрашивает что-то. "
        "Если это вопрос — отвечай question, не win. "
        "Отвечай ТОЛЬКО одним словом."
    )
    result = ai(system, text, max_tokens=5)
    if result:
...
def reframe_win(text):
    system = (
        ...
        "Из текста вытащи ТОЛЬКО те действия которые человек реально написал — не придумывай ничего лишнего. "
        "Одно действие = одна победа. Не разбивай одно действие на несколько пунктов. "
        "Переформулируй каждое как одно короткое тёплое предложение. "
        "Если действий несколько — каждое с новой строки, начинается с символа •. "
        "Если действие одно — просто одно предложение без буллета. "
        "Без пафоса, без слов молодец и браво. Только на русском языке."
    )
    result = ai(system, text)
    return result or f"Ты сделала это: {text}"


def followup_question(count):
    system = (
        "Ты тёплый поддерживающий помощник. Человек только что записал маленькую победу. "
        "Задай ОДИН короткий вопрос — что ещё было сегодня. "
        "Тон лёгкий и ненавязчивый, без давления. "
        "Только один вопрос, одно короткое предложение на русском языке."
    )
    result = ai(system, f"записана победа номер {count}", max_tokens=40)
    return result or "Что ещё было сегодня?"


def respond_to_struggle(text):
    system = (
        "Ты тёплый и спокойный помощник. Человек написал что ничего не делал или устал. "
        "Это нормально — иногда отдыхать и ничего не делать тоже нужно. "
        "Не жалей человека и не драматизируй. Ответь легко и тепло — признай что отдых это ок. "
        "Затем задай ОДИН простой вопрос чтобы вспомнить хоть что-то из дня: что ел, что смотрел, с кем говорил, куда выходил. "
        "Тон как будто разговариваешь с подругой — легко и без пафоса. "
        "На русском, максимум 2-3 предложения."
    )
    result = ai(system, text)
    return result or "Отдыхать тоже важно, это не пустой день. Что сегодня ела или смотрела что-нибудь?"
...
def make_daily_summary(wins_list):
    ...
    comment = ai(system, wins_text, max_tokens=150)
    n = len(wins_list)
...
def make_period_summary(wins_list, period_name):
    ...
    comment = ai(system, wins_text, max_tokens=200)
...
def handle_message(user_id, text):
    ...
    if text.lower() in ["/старт", "/start", "начать"]:
        send(user_id,
             ...
             "Если напишешь что ничего не делала — не буду записывать, просто поговорим.\n\n"
             "Команды:\n"
             "/итог — всё за сегодня\n"
    ...
    if msg_type == "win":
        ...
        question = followup_question(count)
        ...
        if wins:
            ...
            send(user_id, "Сегодня пока нет записей. Напиши что-нибудь — даже самое маленькое.")
    else:
...
def reminders_loop():
    ...
    while True:
        ...
        if h == 0 and m == 0:
            sent_today = {}
        if h == 17 and m == 0:  # 17:00 UTC = 20:00 Moscow
            for uid in get_all_users():
                ...
                if wins:
                    ...
                    system = (
                        "Ты тёплый и лёгкий помощник. Вечер, человек ничего не написал за день. "
                        "Напиши короткое тёплое сообщение без давления и упрёков. "
                        "Признай что бывают разные дни. Задай один простой вопрос про день. "
                        "На русском, максимум 2-3 предложения."
                    )
                    msg = ai(system, "вечернее напоминание") or "Как прошёл день? Расскажи что-нибудь — даже одну маленькую вещь."
                try:
                    send(int(uid), msg)
